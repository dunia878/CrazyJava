...
public class CopyOfReadHtml {
	...
	private String getHtml(String urlString) {
		...
		try (FileWriter fw = new FileWriter(file, true);) {
			...
			while ((-1 != (length = isr.read(buffer, 0, buffer.length)))) { // 按行读取输出流
				...
				fw.append(new String(buffer,0,length)); // 读完每行后换行
			}
			//br.close(); // 关闭
			isr.close(); // 关闭
	...
	public static void main(String[] args) throws IOException {
		...
		File file = new java.io.File("c:\\tids.txt");
		CopyOfReadHtml crh = new CopyOfReadHtml();
		int i = 0;
		try (FileWriter fw = new FileWriter(file);) {
			for (String tid : tids) {
				System.out.println(++i);
				crh.getHtml("http://www.itokoo.com/read.php?tid=" + tid);
			}
		}
	}
